<div>
    @if($data && count($data) )
        <div class="container py-3 md:py-5">
            ...
            @if( count($data)< 4 )
                <div class="row">
                    @foreach($data as $key => $i) @livewire('singleVideoTestimonialItem', [ "i" => $i, key($key) ]) @endforeach
                </div>
            @else
                <div class="swiper-container py-5 relative" x-ref="container" x-data="{swiper: null}" x-init="swiper = new Swiper($refs.container, { loop: true, slidesPerView: 1, spaceBetween: 0, breakpoints: {
                    ...
                    <div class="swiper-wrapper">
                        @foreach($data as $i) @livewire('singleVideoTestimonialItem', [ "i" => $i, key($key) ]) @endforeach
                    </div>
                </div>
            @endif
        </div>
