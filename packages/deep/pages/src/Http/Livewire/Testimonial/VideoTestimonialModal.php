...
class VideoTestimonialModal extends Component
{
    use WithFileUploads;

    public $isOpen = 0, $model, $model_id, $model_options = [], $name, $url, $role, $image, $old_image, $media_id, $testis, $status, $display_order, $data_id;

    public function render(){ return view('deep::livewire.testimonial.video-testimonial-modal')->layout('layouts.admin'); }
    ...
    public function submit(){
        ...
        DB::transaction(function () {
            ...
            VideoTestimonial::updateOrCreate(['id' => $this->data_id], [
                "model"                 =>  $this->model,
    ...
    protected $listeners = [ 'createVideoTestimonialModal', 'openVideoTestimonialModal', 'updateVideoTestimonialModal', 'refreshComponent'=> '$refresh' ];

    public function createVideoTestimonialModal( $model, $id ){
        $this->resetInputFields();
        $this->model                        =   $model;
    ...
    public function openVideoTestimonialModal(){
        $this->resetInputFields();
        $this->status                       =   1;
        $this->isOpen                       =   1;
        $this->getModalOptions();
    ...
    public function updateVideoTestimonialModal( $id ){
        $check                               =   VideoTestimonial::where('id', decode( $id ))->first();
        if( !$check ){ return; }
