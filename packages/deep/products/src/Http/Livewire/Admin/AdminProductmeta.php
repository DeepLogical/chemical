...
class AdminProductmeta extends Component
{
    use WithPagination;
    use WithFileUploads;
    ...
    public function render(){
        $query =   Productmeta::search($this->search)->orderBy('id', 'DESC');
        ...
        return view('deep::livewire.admin.admin-productmeta', [ 'data' => $data ] )->layout('layouts.admin');
    }
    ...
    public function submit(){
        ...
        DB::transaction(function () {
            ...
            if( $this->image ){
                $this->media_id = addOrUpdateSingleImage($this->image, 'productmeta', $this->name, $url, $this->media_id );
            }

            $entry = Productmeta::updateOrCreate(['id' => $this->data_id], [
                'url'               =>  $url,
                'type'              =>  $this->type,
            ...
            $check = Productmeta::select('url')->findOrFail($entry->id);
            if($check){
                addOrUpdateMeta( $check->url, $this->title, $this->description, $this->meta_id, $this->media_id );
            }
            
            $this->dispatch('alert', ['type' => 'success',  'message' => $this->data_id ? 'Product Meta Updated Successfully.' : 'Product Meta Created Successfully.', ]);
            $this->closeModal();
        }, 3);
    }
    ...
    public function edit($id){
        ...
        $check                          =   Productmeta::where('id', $this->data_id)->first();
