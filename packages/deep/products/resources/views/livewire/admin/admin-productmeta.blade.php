<div>
    @livewire('adminSearch', ['page' => 'Admin Product Meta', 'routeName' => '', 'routeText' => 'Add Product Meta', 'link' => 'true'])
        ...
        <tbody>
            @foreach($data as $i)
                <tr wire:key="data-{{ $loop->index }}">
                    ...
                    <td>@if($i->Products) {{ count( $i->Products ) }} @endif</td>
        ...
        <div class="bg-dark fixed h-full left-0 top-0 w-full z-50">
            <div class="bg-white animated fade border bottom-0 fixed h-70 md:h-full md:w-1/2 overflow-y-auto right-0 w-full z-20">
                <div class="flex bg-gray-100 border-b py-2 px-2 items-center justify-center">
                    <h2 class="font-bold w-full">Add Update Product Meta Here</h2>
                    <img src="{{ asset('/images/icons/admin/cross.svg') }}" class="ml-auto hover:cursor-pointer h-6 w-6" wire:click="closeModal()">
                </div>
